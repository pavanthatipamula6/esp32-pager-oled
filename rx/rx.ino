...
void setup() {
  ...
  self_priority = 9199999;
  Serial.printf("This device's priority: %lu\n", self_priority);
  //display oled data
  displayText(0, 0, "MAC");
  displayPrint(WiFi.macAddress());
  displayText(0, 10, "Channel:");
  displayPrint((String)ESPNOW_WIFI_CHANNEL);
  displayText(0, 20, "Priority:");
  displayPrint((String)self_priority);
  delay(2000);
  //end display data
...
void sendSerialData() {
  if (Serial.available() > 0) {
    ...
    input.trim();
    // strncpy(new_msg.str, msg.c_str(), sizeof(new_msg.str));

    if (input.length() > 0) {
      strncpy(new_msg.str, input.c_str(), sizeof(new_msg.str));
      new_msg.count = ++sent_msg_count;
      Serial.print("data input :\t");
      Serial.println(new_msg.str);
      for (auto &peer : peers) {
